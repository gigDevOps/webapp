        ...
        <>
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
                <InputGroup label="Shift Start">
                    <input ref={register({ required: true })} name="shiftstartdate" placeholder="Start Date" type="date"/>
                    <input ref={register({ required: true })} name="shiftstarttime" placeholder="Start Time" type="time"/>
                </InputGroup>
                <InputGroup label="Shift End">
                    <input ref={register({ required: true })} name="shiftenddate" placeholder="End Date" type="date"/>
                    <input ref={register({ required: true })} name="shiftendtime" placeholder="End Time" type="time"/>
                </InputGroup>
                <InputGroup label="Minimum Working Hours" tooltip="Minimum Woking Hours">
                    <input name="minimum_hours" type="integer" ref={register({ required: true })}/>
                    ...
                    {isFetchingLocations && <p>Loading locations...</p>}
                    {
                        !isFetchingLocations && locations && (
                            <select name="location_id" ref={register}>
                                {
                                    locations.map((d) => {
                                        return <option value={d.id}>{d.name} ({d.address})</option>
                                    })
                                }
                            </select>
                        )
